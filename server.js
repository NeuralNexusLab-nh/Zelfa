...
// --- SECURITY CONFIG ---
const ENCRYPTION_KEY = "d7f8a9e2b1c4d3f5a6e8c0b7d9f2a1c5e8b4d3a6f9c0e7b2d5a8f1c4e3b6d9a2"; 
const IV_LENGTH = 16;
...
async function requireUserAuth(req, res, next) {
    ...
    // 24 hours expiry
    if (Date.now() - session.ts > 86400000) {
        res.clearCookie('session_token');
        return res.redirect('/login');
    }
...
// 2. Admin Auth (for Admin Panel)
async function requireAdminAuth(req, res, next) {
    // 這裡不直接 redirect，因為前端 admin.html 會有輸入密碼的介面
    // 若是用戶直接訪問 /api 路徑，則返回 403
    const cookie = req.cookies['admin_token'];
    
    if (!cookie) {
        if(req.path.startsWith('/api')) return res.status(403).json({error: "Admin Auth Required"});
        return next(); // Let the page load, frontend handles the password modal
    }
    ...
    req.isAdmin = true; // Tag for API usage
    next();
}
...
// Admin 頁面不需強制 redirect，內部 JS 會檢查 cookie 並彈出密碼框
...
// 2. Admin Actions (Manage Users)
app.post('/api/admin/users', requireAdminAuth, async (req, res) => {
    if (!req.isAdmin) return res.status(403).json({ error: 'Admin Access Only' });
    ...
    if (action === 'create') {
        ...
        users[username] = {
            ...
            limits: limits || {
                "gpt-5-mini": 50, "deepseek-v3": 50, 
                "gemini-2.0-flash": 50, 
                "gpt-5-nano": 50, "gpt-5.1-codex-mini": 50 
            },
...
app.post('/api/login', async (req, res) => {
    ...
    if (!users[username]) return res.status(401).json({ error: 'User not found' });
...
// 5. Chat Actions (Load/Rename/Delete)
app.post('/api/chat/load', requireUserAuth, async (req, res) => {
    ...
    if (chatId) {
        ...
        if (!chat || chat.owner !== req.user) return res.status(403).json({ error: 'Access Denied' });
...
app.post('/api/chat/action', requireUserAuth, async (req, res) => {
    ...
    if (!history[chatId] || history[chatId].owner !== req.user) return res.status(403).json({ error: 'Access Denied' });
...
app.post('/api/models', requireUserAuth, async (req, res) => {
    ...
    // Rate Limit
    if (!(await checkRateLimit(username, model))) {
    ...
    try {
        // Provider Logic
        if (model.includes('gpt-5-mini') || model.includes('deepseek')) {
            // ChatAnyWhere
            const apiRes = await fetch("https://api.chatanywhere.tech/v1/chat/completions", {
                ...
                body: JSON.stringify({
                    model: model,
                    messages: [
                        { role: "system", content: "You are GPT-5-Mini model and developed by OpenAI. Answer helpfully." },
            ...
            // OpenAI (Simulated Endpoint per request)
            const apiRes = await fetch("https://api.openai.com/v1/responses", {
                ...
                body: JSON.stringify({ model: model, input: finalInput })
            });
            const data = await apiRes.json();
...
app.listen(process.env.PORT, () => console.log(`Zelfa Core Online: ${process.env.PORT}`));
